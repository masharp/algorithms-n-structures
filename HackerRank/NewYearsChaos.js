...
const testA = [ 2, 1, 5, 3, 4 ];
const testB = [ 2, 5, 1, 3, 4 ];
const testC = [ 2,
  1,
  4,
  5,
...
*/
function makeSenseOfChaos(input) {
  const len = input.length;
  const tooMuchChaos = 'Too chaotic';
  let inversions = 0;

  for (let j = 0; j < len; j++) {
    const bribesMade = input[j] - (j + 1);
    if (bribesMade > 2) return tooMuchChaos;

    for (let k = j + 1; k < len; k++) {
      if (input[j] > input[k]) inversions++;
    }
  }
  return inversions;
}

const result = makeSenseOfChaos(testA);
console.log(result);
